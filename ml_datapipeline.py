...
class DataPipeline:
    def __init__(self, scale: bool = True, augment_feature: bool = False, config_path: str="config.yml"):
        ...
        self.debug = False # flag to initiate debug mode 
    ...
    def detect_n_create_new_dataset(self, dataset):
        """Check for new datasets & process them."""
        folder_path = os.path.join(self.data_dir, dataset)
        if DataPipeline.check_folder_files(folder_path, self.file_patterns) and self.debug:
            import shutil
    ...
    def store_data(self, X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray,\
        ...
        pd.concat([pd.DataFrame(X_train, columns=[f"feature_{i}" for i in range(X_train.shape[1])]), pd.DataFrame(y_train, columns=['target'])], axis=1).to_csv(f"{output_path}/train.csv", index=False)
        pd.concat([pd.DataFrame(X_test, columns=[f"feature_{i}" for i in range(X_test.shape[1])]), pd.DataFrame(y_test, columns=['target'])], axis=1).to_csv(f"{output_path}/test.csv", index=False)
    ...
    def get_data(self, dataset: str='synthetic', debug: bool=False) -> list:
        ...
        if self.detect_n_create_new_dataset(dataset):
            ...
            return self.preprocess_and_split(X, y)
        else:
            raise ValueError or IOError or AttributeError
